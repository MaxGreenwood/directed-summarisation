...
# EDITED FOR DATAFRAMES
def get_resolved_clusters(location_of_resolved_story='',location_of_story_in_file=0):
	resolved_stories = pd.read_csv(location_of_resolved_story, converters={"sentences": literal_eval, "predicted_clusters": literal_eval, "head_scores": literal_eval} )
	resolved_story = resolved_stories.iloc[location_of_story_in_file]
	clusters = resolved_story['predicted_clusters']
	story = " ".join(resolved_stories.iloc[location_of_story_in_file]['sentences'])
	story = nltk.tokenize.word_tokenize(story)
	attention_weights = resolved_story['head_scores']
...
def resolve_coref_doc_AMR(amr,resolved=True,story='',
	...
	for cluster in clusters:
		...
		if first_var != '':
			# print(first_var, ' -> ', ''.join([word.split('/')[0] for word in (' '.join(largest_reference))]))
			try:
				# idf_dict[first_var] = idf[' '.join([word.split('/')[0] for word in lemmatizer.lemmatize(' '.join(largest_reference))])]
				idf_dict[first_var] = idf[''.join([word.split('/')[0] for word in lemmatizer.lemmatize(' '.join(largest_reference))])]
			except KeyError:
...
if __name__=='__main__':
	...
	# import csv
	# results = {}
	# with open('amr_proxy_coref.csv', 'r') as pscfile:
	# 	reader = csv.DictReader(pscfile)
	# 	for row in reader:
	# 		results['coref'] = row['coref']
	# print(results)


	clusters, story, attention_weights = get_resolved_clusters('amr_proxy_coref.csv')
	words_corresponding_to_clusteres(story, clusters, attention_weights)