# Graph visualisation adapted from:
# https://github.com/xdqkid/AMR-Visualization/blob/master/amr-slide-simple.py
...
def produce_graph(penman):
    amr_lines = []
    amr_count = 0
    lines = penman.splitlines()
    for line in lines:
        if line.startswith('#'):
            continue
        if line.strip() == '':
            if len(amr_lines) != 0:
                _ = convert2graph(amr_lines, amr_count)
                amr_count += 1
                amr_lines = []
            continue
        amr_lines += [line.strip()]
    if len(amr_lines) != 0:
        graph = convert2graph(amr_lines, amr_count)
    return graph
...
def convert2graph(amr_lines, amr_count):
    ...
    for key in concept_map.keys():
        concept_map[key] = concept_map[key] \
                            .replace('&lt;', '(') \
                            .replace('&gt;', ')')
    graph = draw(relations, concept_map, amr_count)
    return graph
