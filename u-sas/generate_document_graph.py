...
# 	2. makes sure that variable names are different across different sentences
...
def merge_sentence_amrs(amr_list,debug=False):
	...
	doc_amr = []
	# add root node, this is just for AMR representation using AMRICA (graph visualisation)
	...
	for sent_index_in_doc, sent_amr in enumerate(amr_list):
		...
		if len(cycles) != 0:
			...
			# COMMENTED OUT (connected_components is 'generator' object and has no attribute 'len')
		# if len(connected_components) > 1: 
			...
			for node in list(sent_amr.directed_graph._graph.keys()):
				if node not in largest_component:	
					sent_amr.directed_graph.remove(node=node)
