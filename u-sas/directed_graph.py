...
class Graph(object):
    ...
    def construct_greedily_first(self,ranks=[],weights=[],use_true_sent_rank=False,num_nodes=0,range_num_nodes=0,
        ...
        if len(top_nodes) <= 5:  top_nodes = list(reversed(list(ranks[-20:]))) # Take the highest ranked nodes (n=10)
        else: top_nodes = list(reversed(list(ranks[-20:])))
        ...
        # if (set(top_nodes) < set(selected_nodes) or set(top_nodes) == set(selected_nodes)):
        #     print('Ran out of top nodes')
    ...
    def rank_sent_in_degree(self,):
        ...
        for key in self.sent_in_degree.keys():
            min_sent = 10000
            try:
                min_sent = min(self.var_to_sent[key]) # takes minimum sentence position as priority
            except: pass
    ...
    ##################################################################################
    def add_mft_ranking(self, var_freq_list=[], mft_factor=5, mft_vars={}):
        new_list = []
        for index, entry in enumerate(var_freq_list):
            if entry[0] in mft_vars.keys():
                new_list.append((entry[0], entry[1] * mft_factor, entry[2])) # Multiply existing weight by MFT factor if MFT variable
            else:
                new_list.append((entry[0],entry[1],entry[2]))
        new_list.sort(key=lambda t: (t[1],-t[2]))
        return new_list
    ...
    def add_mft_embed_ranking(self, var_freq_list=[], mft_embed_factor=5, mft_embed_vars={}, sent_first_ranking=False):

        # Create sentence prioritisation dictionary (negative values for ranking purposes)
        sent_priority = {}
        unique_sent_ids = list(set(list(mft_embed_vars.values())))
        unique_sent_ids.sort(key=lambda t: t[0])
        for rank, sent_id in enumerate(unique_sent_ids):
            sent_priority[sent_id[1]] = 0-(rank)

        new_list = []
        for index, entry in enumerate(var_freq_list):
            if entry[0] in mft_embed_vars.keys():
                new_sent_rank_value = sent_priority[mft_embed_vars[entry[0]][1]]
                new_list.append((entry[0], entry[1] * mft_embed_factor, new_sent_rank_value))
            else:
                new_list.append((entry[0],entry[1],entry[2]+1))

        # Choose word/sent priority order
        if not sent_first_ranking:
            new_list.sort(key=lambda t: (t[1],-t[2]))

        if sent_first_ranking:
            new_list.sort(key=lambda t: (-t[2],t[1]))
    ...
    def max_in_degree(self,):
        ...
        node_key = max((self.in_degree).items(), key=operator.itemgetter(1))[0]
        return node_key

    def second_max_in_degree(self,):
        """Returns the node with the second maximum incoming arcs/edges"""
        temp_graph = self.in_degree.copy()        
        max_in_degree = max((temp_graph).items(), key=operator.itemgetter(1))[0]
        del temp_graph[max_in_degree]
        node_key = max((temp_graph).items(), key=operator.itemgetter(1))[0]
        return node_key
    ...
    def check_sent_var_mapping(self,):
        ...
        self.sent_to_var = self.get_sent_to_var()
        for sent,sent_vars in self.sent_to_var.items():
        ...
        if self.root == '':
            ...
            for node in self.reverse_graph.keys():
                if len(self.reverse_graph[node]) == 0:
                    self.root = node 
                    break
        if self.root == '': self.root = list(self._graph.keys())[0]
    ...
    def remove(self, node,reconstruct_nx=False):
        ...
        for n, cxns in self._graph.items():
            while node in cxns:
                cxns.remove(node)
        for n, cxns in self.reverse_graph.items():
            while node in cxns:
                cxns.remove(node)
        for n, cxns in self.text_index_to_var.items():
            while node in cxns:
                cxns.remove(node)
