...
def read_data(location_data, story_type='tok'):
	with codecs.open(location_data, 'r', encoding='utf-8') as data_file:
	...
	for index, line in enumerate(data):
		...
		if '::snt-type summary' in line:
			temp_line = data[index+plus_idx].split(f'::{story_type}')[1]
	...
	for index, doc in enumerate(docs):
		...
		for line in doc:
			...
			if '::id' in line:
				new_dict['id'] =  line.split('id')[1].split(' ')[1].strip()
			if read_amr:
				if line == '\n':
					read_amr = False
					start_reading = False
